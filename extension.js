	...
	let create_new_ccf_application_javascript = vscode.commands.registerCommand('mccf-vscode-extension.javascriptapp', async function () { // used async becuase ome operations in the function, such as showing input boxes and performing git cloning, may involve asynchronous operations that return things
		...
		const template = await vscode.window.showQuickPick( 
			[
				...
				{ label: 'Custom', description: 'Create a CCF application from a custom template.' },
			],
		...
		if (template.label === 'Empty') {
			...
			const templateOptions = [
				{ label: 'Audiable Logging App', description: 'This is a sample application of logging app that takes advantage of CCFs ability for granular access control.', repository: 'https://github.com/microsoft/ccf-app-samples.git/auditable-logging-app' },
				{ label: 'Banking App', description: 'This is a sample application of a bank consortium.', repository: 'https://github.com/microsoft/ccf-app-samples.git/banking-app' },
				{ label: 'Data Reconciliation App', description: 'This is the CCF Data Reconciliation - sample in typescript', repository: 'https://github.com/microsoft/ccf-app-samples.git/data-reconciliation-app' }
			  ];

			// Show QuickPick menu to choose custom template
			const templateSelection = await vscode.window.showQuickPick(templateOptions, {
        	...
        	vscode.window.showInformationMessage(`Selected custom template: ${selectedTemplate}, App name: ${appName}`);
		}
