...
ccf_cose_sign1 --ccf-gov-msg-type ballot --ccf-gov-msg-proposal_id $proposal0_id --ccf-gov-msg-created_at `date -uIs` --signing-key member0_privk.pem --signing-cert member0_cert.pem --content ../../samples/ballots/vote_accept.json | curl -s "$proposalUrl/$proposal0_id/ballots" --cacert service_cert.pem --data-binary @- -H "content-type: application/cose" | jq
...
ccf_cose_sign1 --ccf-gov-msg-type ballot --ccf-gov-msg-proposal_id $proposal0_id --ccf-gov-msg-created_at `date -uIs` --signing-key member2_privk.pem --signing-cert member2_cert.pem --content ../../samples/ballots/vote_accept.json | curl -s "$proposalUrl/$proposal0_id/ballots" --cacert service_cert.pem --data-binary @- -H "content-type: application/cose" | jq
