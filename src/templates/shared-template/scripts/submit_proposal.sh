...
ccf_cose_sign1 --ccf-gov-msg-type ballot --ccf-gov-msg-proposal_id $proposal0_id --ccf-gov-msg-created_at `date -uIs` --signing-key $signing_key --signing-cert $signing_cert --content ${app_dir}/samples/ballots/vote_accept.json | curl -s "$network_url/gov/proposals/$proposal0_id/ballots" --cacert $service_cert --data-binary @- -H "content-type: application/cose" | jq
