...
proposal0_out=$(ccf_cose_sign1 --ccf-gov-msg-type proposal --ccf-gov-msg-created_at "$(date -Is)" --signing-key "$signing_key" --signing-cert "$signing_cert" --content "$proposal_file" | curl "$network_url/gov/proposals" --cacert "$service_cert" --data-binary @- --header "content-type: application/cose" --silent)
