...
function activate(context) {
  ...
  let install_dev_container = vscode.commands.registerCommand(
    ...
    async function () {
      ...
      if (template.label === "Empty") {
        ...
        // Install additional extension(s) in the dev container
        const extensionId = "tonybaloney.vscode-pets";
      ...
      //FIXME: This is not working
        ...
        const link =
          "vscode://ms-vscode-remote.remote-containers/cloneInVolume?url=" +
          selectedTemplate;
  ...
  let start_ccf_network = vscode.commands.registerCommand(
    "mccf-vscode-extension.startNetwork",
    function () {
      // Run the ccf commands to start the network
      try {
        // Install dependencies
        execSync("npm install dockerode");

        //Creates a new instance of the Docker Class
        const Docker = require("dockerode");
        ...
        const imageConfig = {
          ...
          run: [
            'apt-get update',
            'apt-get install -y curl build-essential cmake git libssl-dev libuv1-dev pkg-config python3 python3-pip',
            'git clone https://github.com/microsoft/CCF.git',
            'cd CCF && mkdir build && cd build && cmake .. && make -j'
          ],
        ...
        function buildDockerImage() {
          // Docker buildImage logic
          docker.buildImage(imageConfig, {}, (err, stream) => {
            if (err) {
            ...
            stream.on("data", (chunk) => {
              // Handle build progress information, if needed
        ...
        // Call the buildDockerImage function
        buildDockerImage();
